...
def run_js_in_chrome(js: str) -> str:
    ...
    script = f'tell application "Google Chrome" to execute active tab of window 1 javascript "{js_escaped}"'
    result = subprocess.run(["osascript", "-e", script], capture_output=True, text=True)
...
def parse_caption(meta_path: Path) -> str:
    ...
    # Description ends at next ## section, a --- divider, or end of file
...
""")
    ...
    # ── Enter the file path in the OS file picker via AppleScript ────────────
    # AppleScript System Events is more reliable than pyautogui here because
    # pyautogui sends keys to whatever window happens to be frontmost, while
    # AppleScript targets the Chrome process directly.
    print(f"  Entering file path via AppleScript: {video_path}")
    abs_path = str(video_path.resolve())
    # Escape backslashes and quotes for AppleScript string
    abs_path_escaped = abs_path.replace("\\", "\\\\").replace('"', '\\"')
    picker_script = f"""
tell application "System Events"
    tell process "Google Chrome"
        -- open Go To Folder sheet (works in macOS file picker)
        keystroke "g" using {{command down, shift down}}
        delay 1.5
        -- select-all + type full path (more reliable than paste in Go To Folder)
        keystroke "a" using command down
        delay 0.2
        keystroke "{abs_path_escaped}"
        delay 0.5
        key code 36
        delay 1.0
        key code 36
...
"""
    result = subprocess.run(["osascript", "-e", picker_script], capture_output=True, text=True)
    if result.returncode != 0:
        print(f"  AppleScript picker error: {result.stderr.strip()[:200]}")
        return False
    time.sleep(2)
    print("  File selected.")
    return True
...
def fill_caption(caption: str):
    """Click the TikTok caption field with pyautogui and paste the caption."""
    print("  Filling caption...")

    if not HAS_PYAUTOGUI:
        print("  Warning: pyautogui not installed — cannot fill caption reliably.")
        return

    # Copy to clipboard first so it's ready
    subprocess.run(["pbcopy"], input=caption.encode("utf-8"), check=True)
    time.sleep(0.3)

    # Find caption field screen coordinates via JS (check main frame + iframes)
    find_caption_js = """
(function() {
    var selectors = [
        ...
        if (el && el.offsetParent !== null) {
            ...
            if (r.width > 50 && r.height > 10) {
                ...
                return (window.screenX + Math.round(r.left + r.width / 2)) + ',' +
                       ...
                       '|matched:' + selectors[i].slice(0, 40);
            }
        }
    }
    return 'not found';
...
"""
    coords_raw = run_js_in_frame(find_caption_js)
    print(f"  Caption field: {coords_raw}")

    coord_part = coords_raw.split("|")[0]
    if "," in coord_part and not coord_part.startswith("not found"):
        # Real OS-level click via pyautogui — guarantees keyboard focus
        if _click_element_by_coords(coord_part, "caption field"):
            time.sleep(0.6)
            pyautogui.hotkey("command", "a")   # select all existing text
            time.sleep(0.2)
            pyautogui.hotkey("command", "v")   # paste caption
            time.sleep(0.8)
            print(f"  Caption pasted ({len(caption)} chars): {caption[:80]}{'...' if len(caption) > 80 else ''}")
            return

    # Fallback: try clicking via JS + AppleScript paste
    print("  Caption field coords not found — trying JS focus + AppleScript paste fallback")
    run_js_in_chrome("""
(function() {
...
""")
    time.sleep(0.8)
    print(f"  Caption paste attempted via fallback ({len(caption)} chars)")
...
def main():
    ...
    # Find thumbnail: new format (_01_thumb.png) or legacy (_thumbnail.png)
    thumbnail = None
    for f in sorted(folder.iterdir()):
        if f.is_file() and re.search(r'_(thumb|thumbnail)\.(png|jpg|jpeg|webp)$', f.name.lower()):
            thumbnail = f
            break
